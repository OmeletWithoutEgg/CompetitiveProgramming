...
signed main() {
    ...
    int n = (1<<h) - 1;
    // ask(u, v, w) = 
    //      u, if w is in subtree of u
    //      v, if w is in subtree of v
    //      a node on path u-v

    // make a, b be in the diffrent subtree of the root's direct childs
    int a = 1, b = 2;
    for(int i = 1; i <= 100; i++) {
        int v;
        do {
    ...
    // then take nodes on the path a, b
    vector<int> path;
    for(int i = 1; i <= n; i++)
        if(i != a && i != b && ask(a, b, i) == i)
            path.pb(i);
    // and then check what is this chain?
}
