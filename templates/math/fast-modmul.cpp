...
class Mint {
   ...
   public:
    ...
    template <typename U>
    Mint(const U& u) {
        v = mont.from(0 <= u && u < MOD ? u : (u % MOD + MOD) % MOD);
    }
    template <typename U>
...
#define REFOP(type, expr...) \
    Mint& operator type(const Mint& rhs) { return expr, *this; }
    REFOP(+=, v += rhs.v - MOD, v += MOD & (v >> width))
    REFOP(-=, v -= rhs.v, v += MOD & (v >> width))
    // fits for MOD^2 <= 9e18
    REFOP(*=, v = mont.redc(v, rhs.v))
#define VALOP(op) \
    friend Mint operator op(Mint a, const Mint& b) { return a op## = b; }
    VALOP(+)
    VALOP(-)
    VALOP(*)
    ...
    friend std::istream& operator>>(std::istream& I, Mint& m) {
        T x;
        I >> x, m = mont.from(x);
        return I;
    }
    friend std::ostream& operator<<(std::ostream& O, const Mint& m) {
        return O << mont.get(m.v);
    }
