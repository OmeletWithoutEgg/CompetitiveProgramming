...
signed main() {
    ...
    for(int i = 0; i < m; i++) {
        ...
        --l;
        /* mem(p, 0); */
        for(int j = 0; j < min(k, r-l); j++) {
            int x = clamp(l-(k-1-j), 0, n);
            int y = clamp(r-j, 0, n);
            p[0][x] += 1;
            p[0][y] -= 1;
            p[x][0] += 1;
        ...
        debug(l, r, k);
        for(int j = 0; j < n; j++) {
            int len = max(0, min(j+k, r) - max(j, l));
            a[i][j] = len;
        }
        for(int x = 0; x < n; x++) {
            for(int y = 0; y < n; y++) {
        ...
        for(int x = 0; x < n; x++) {
            for(int y = 0; y < n; y++) {
                b[x][y] = max(a[i][x], a[i][y]);
                cerr << (b[x][y] - (x?b[x-1][y]:0) - (y?b[x][y-1]:0) + (x&&y?b[x-1][y-1]:0)) << ' ';
            }
            cerr << '\n';
        }
        cerr << '\n';
    ...
    for(int i = 0; i+k <= n; i++) for(int j = 0; j+k <= n; j++) {
        p[i][j] += (i ? p[i-1][j] : 0) + (j ? p[i][j-1] : 0) - (i&&j ? p[i-1][j-1] : 0);
        ans = max(ans, p[i][j]);
    }
    cout << ans << '\n';
}
