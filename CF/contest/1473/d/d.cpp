...
struct RMQ {
    ...
    void init(int v[], int _n) {
        n = _n;
        for(int i = 0; i < n; i++) mx[i+n] = mn[i+n] = v[i];
        for(int i = n-1; i > 0; i--) {
...
void solve() {
    ...
    while(m--) {
        int l, r;
        cin >> l >> r;
        auto [mxl, mnl] = sgt.query(0, l);
        auto [mxr, mnr] = sgt.query(r+1, n+1);
        mxr -= v[r] - v[l-1], mnr -= v[r] - v[l-1];
        int mx = max(mxl, mxr), mn = min(mnl, mnr);
        cout << mx-mn+1 << '\n';
    }
}
signed main() {
