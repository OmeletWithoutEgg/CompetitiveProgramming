...
constexpr ll N = 800025, INF = 1e18, MOD = 1000000007, K = 14699, inf = 1e9;
...
signed main() {
    ...
    int n, m;
    cin >> n >> m;
    auto encode = [](int a, int b) { return a << 2 | b; };
    for(int i = 0; i < m; i++) {
        int a, b, w;
        cin >> a >> b >> w;
        --a, --b;
        for(int t = 0; t < 2; t++) {
            addEdge(encode(a, 0), encode(b, 0), w);
            addEdge(encode(a, 1), encode(b, 1), w);
            addEdge(encode(a, 2), encode(b, 2), w);
            addEdge(encode(a, 3), encode(b, 3), w);

            addEdge(encode(a, 0), encode(b, 1), 0);
            addEdge(encode(a, 0), encode(b, 2), w*2);
            addEdge(encode(a, 1), encode(b, 3), w*2);
            addEdge(encode(a, 2), encode(b, 3), 0);
            addEdge(encode(a, 0), encode(b, 3), w);
            swap(a, b);
        }
    }
    dis[encode(0, 0)] = 0;
    pq.emplace(encode(0, 0), 0);
    while(!pq.empty()) {
