...
signed main() {
    ...
    for (int i = 0; i < m; i++) {
        while (!pq.empty() && get<0>(pq.top()) <= t[i]) {
            auto [d, c, s] = pq.top(); pq.pop();
            if (dis[c][s] != d) continue;
            for (int j = 0; j < n; j++) {
                if (~s >> j & 1) {
                    int nd = dis[c][s] + nearest[j][s];
                    int ns = s | (1<<j);
        ...
        for (int s = 0; s < (1<<n); s++) {
            ...
            for (int j = 0; j < n; j++) {
                if (~s >> j & 1) {
                    int nd = t[i] + min(nearest[j][s], abs(x[j] - x[i+n]) + abs(y[j] - y[i+n]));
                    int ns = s | (1<<j);
                    if (chmin(dis[c][ns], nd))
                        pq.emplace(dis[c][ns], c, ns);
                }
            ...
            for (int j = i+1; j < m; j++) {
                int nd = t[i] + min(nearest[j+n][s], abs(x[i+n] - x[j+n]) + abs(y[i+n] - y[j+n]));
                if (nd > t[j]) continue;
                // assert( c < m );
                if (canReach[j][s] < c+1) {
                    canReach[j][s] = c+1;
