...
signed main() {
    ...
    while (t--) {
        ...
        if (a != b) {
            int g = modpow(10, c-1)();
            int x = modpow(10, a-1)();
            int y = modpow(10, c-1)() * (b==c ? 1 : modpow(10, b-c)() + 1);
            debug(a, b, c, x, y, g);
            assert(__gcd(x, y) == g);
            cout << x << ' ' << y << '\n';
        } else {
            int L = modpow(10, a-c)();
            int x = uniform_int_distribution<int>(L, L*10-1)(rng);
            int y = uniform_int_distribution<int>(L, L*10-1)(rng);
            do {
                x = uniform_int_distribution<int>(L, L*10-1)(rng);
                y = uniform_int_distribution<int>(L, L*10-1)(rng);
            } while (__gcd(x, y) != 1);
            int g = modpow(10, c-1)();
            cout << x * g << ' ' << y * g << '\n';
            assert(__gcd(x*g, y*g) == g);
        }
    }
}
