...
signed main() {
    ...
    vector<Mint> val(max(n, m)+1);
    for (int h = 3; h <= max(n, m); h++) {
        debug(h);
        val[h] = val[h-1];
        if (h % 2 == 1) {
            val[h] += modpow(2, white - h);
        }
        /*
        for (int c = h-2; c >= 1; c -= 2) {
            debug(modpow(2, white - (h-c+1)));
            val[h] += modpow(2, white - (h - c + 1));
        }
        */
    }
    Mint ans = 0;
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            if (v[i][j]) {
                ...
                debug(white - hor[i][j]);
                if (hor[i][j] % 2 == 0)
                    ans += modpow(2, white - hor[i][j]);
                ans += val[hor[i][j]];
                /*
                for (int c = hor[i][j] - 2; c >= 1; c -= 2) {
                    ans += modpow(2, white - (hor[i][j] - c + 1));
                }
                */
            }
        }
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            if (v[i][j]) {
                ...
                if (ver[i][j] % 2 == 0)
                    ans += modpow(2, white - ver[i][j]);
                ans += val[ver[i][j]];
                /*
                for (int c = ver[i][j] - 2; c >= 1; c -= 2) {
                    ans += modpow(2, white - (ver[i][j] - c + 1));
                }
                */
            }
        }
