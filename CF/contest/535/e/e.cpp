...
signed main() {
    ...
    auto ok = [](pair<int,int> A, pair<int,int> B, pair<int,int> C) -> bool {
        // returns m(AB) <= m(BC)
        /* return (1.0/B.second - 1.0/A.second) * (1.0/C.first - 1.0/B.first) > (1.0/C.second - 1.0/B.second) * (1.0/B.first - 1.0/A.first); */
        /* return ((A.ss - B.ss) / (A.ss * B.ss)) * ((B.ff - C.ff) / (C.ff * B.ff)) > ((B.ss - C.ss) / (B.ss * C.ss)) * ((A.ff - B.ff) / (A.ff * B.ff)) */
        /* return ((A.ss - B.ss) / A.ss) * ((B.ff - C.ff) / C.ff) > ((B.ss - C.ss) / C.ss) * ((A.ff - B.ff) / A.ff) */
        return (ll(A.ss - B.ss) * A.ff) * (ll(B.ff - C.ff) * C.ss) > (ll(B.ss - C.ss) * C.ff) * (ll(A.ff - B.ff) * A.ss);
    };
    int n;
    cin >> n;
    vector<pair<int,int>> org(n);
    for(auto &[a, b]: org) cin >> a >> b;
    ...
    for(auto p: v) {
        debug(p);
        while(H.size() >= 2 && ok(H[H.size()-2], H.back(), p)) H.pop_back();
        H.pb(p);
    }
    while(H.size() >= 2 && H[H.size()-2].second >= H.back().second) H.pop_back();
    reverse(all(H));
    while(H.size() >= 2 && H[H.size()-2].first == H.back().first) H.pop_back();
    reverse(all(H));
    set<pair<int,int>> OK(H.begin(), H.end());
    vector<int> ans;
    for(int i = 0; i < n; i++) if(OK.count(org[i])) ans.pb(i+1);
