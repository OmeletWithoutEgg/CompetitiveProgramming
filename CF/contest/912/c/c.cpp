...
constexpr ll N = 1000025, INF = 1e18, MOD = 1000000007, K = 14699, inf = 2e9;
...
signed main() {
    ...
    vector<int> maxH(n), regen(n);
    for(int i = 0; i < n; i++) {
        int startH;
        cin >> maxH[i] >> startH >> regen[i];
        evt[i].pb(0, startH);
    }
    for(int i = 0; i < m; i++) {
        int t, e, h;
    ...
    for(int i = 0; i < n; i++) {
        ...
        for(int j = 0; j < v.size(); j++) {
            ...
            if(H <= D) {
                ...
                int T2 = inf;
                if(regen[i] != 0)
                    T2 = min(T2, int(T + cdiv(D+1-H, regen[i])));
                if(j+1 != v.size())
                    T2 = min(T2, v[j+1].first);
                if(regen[i] != 0 || j+1 != v.size()) pr.pb(T2, -1);
            }
        }
    }
    sort(all(pr));
    int cnt = 0;
    ll ans = 0;
    for(int i = 0, j = 0; i < int(pr.size()); i = j) {
        for(j = i; j < int(pr.size()); j++) {
            if(pr[i].first != pr[j].first) break;
            cnt += pr[j].second;
        }
        if(cnt) {
            if(j != pr.size() || I == 0) {
                int T = pr[j].first - 1;
                ans = max(ans, cnt * (B + T * I));
            } else return cout << -1 << '\n', 0;
        }
    }
    cout << ans << '\n';
