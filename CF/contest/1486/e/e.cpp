...
signed main() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        cin >> a >> b >> w;
        --a, --b;
        addEdge(enc(a, 0), enc(b, w), w * w);
        addEdge(enc(b, 0), enc(a, w), w * w);
        for (int c = 1; c <= 50; c++) {
            addEdge(enc(a, c), enc(b, 0), 2 * c * w + w * w);
            addEdge(enc(b, c), enc(a, 0), 2 * c * w + w * w);
        }
    }
    fill(dis, dis + n*51, INF);
