...
signed main() {
    ...
    while(tc--) {
        ...
        if(a > b*c) cout << -1 << '\n';
            ...
            // 0, d, 2d, ... kd
            // First, kd <= c because a <= b*c
            // td would be added kd - td
            auto calc = [&](ll k) -> ll {
                if(k < 0 || k*d > c) return -INF;
                ll cnt = (k) * (k+1) / 2 * d;
                return a*(k+1) - b*cnt;
                // 2ak + 2a - bd(k^2+k) = -bd(k^2) + (2a - bd)k + 2a
                // -2bdk + (2a-bd) = 0 -> k = (2a-bd) / (2bd) +- 1
            };
            ll ans = -INF;
            ans = max(ans, calc(0));
            ans = max(ans, calc(c/d));
            ans = max(ans, calc((a*2 - b*d) / (2*b*d)));
            ans = max(ans, calc((a*2 - b*d) / (2*b*d) + 1));
            cout << ans << '\n';
        }
    }
}
