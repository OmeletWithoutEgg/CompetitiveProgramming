...
struct Distance {
    ...
    friend bool operator<(const Distance &a, const Distance &b) {
        ...
        if(max(a.E, b.E) < 30) return (1<<a.E) + a.R < (1<<b.E) + b.R;
        return a.E < b.E;
    }
    friend bool operator!=(const Distance &a, const Distance &b) {
    ...
    int eval() const {
        return (modpow(2, E) - 1 + R) % MOD;
    }
} dis[2][N];
template <typename T> bool upd(T &a, T v) {
    return v < a ? (a=v, true) : false;
}
vector<int> g[2][N];
signed main() {
    ...
    for(int i = 1; i <= n; i++) dis[0][i] = dis[1][i] = {inf, 0};
    min_heap<tuple<Distance,int,int>> pq;
    dis[0][1] = {0, 0};
    pq.push({{0, 0}, 0, 1});
    while(!pq.empty()) {
        auto [D, dir, i] = pq.top(); pq.pop();
        if(D != dis[dir][i]) continue;
        if(upd(dis[!dir][i], dis[dir][i] + Distance{1, 0})) {
            pq.push({dis[!dir][i], !dir, i});
        }
        for(int j: g[dir][i]) {
            if(upd(dis[dir][j], dis[dir][i] + Distance{0, 1})) {
                pq.push({dis[dir][j], dir, j});
            }
        }
    }
    cout << min(dis[0][n], dis[1][n]).eval() << '\n';
}
