...
void solve() {
    ...
    cin >> n >> k >> s;
    vector<int> sum(n);
    for(int i = 0; i < n; i++) sum[i] = (i ? sum[i-1] : 0) + !(s[i] - '0');
    auto query = [&](int l, int r) { return (r ? sum[r-1] : 0) - (l ? sum[l-1] : 0); };
    // at most n-k+1 would be banned
    // enumerate more
    // check if 2^k <= n-k+1
    while((1<<L) <= n) ++L;
    if(k < L) L = k;
    vector<bool> app(1 << L);
    int x = bin(s.substr(k - L, L)), U = (1 << L) - 1;
    if(query(0, k-L) == 0)
        app[U ^ x] = true;
    debug(x);
    for(int i = k; i < n; i++) {
        ...
        x |= s[i] - '0';
        x &= U;
        if(query(i-k+1, i-L+1) == 0)
            app[U ^ x] = true;
        debug(x);
    }
    for(int i = 0; i < (1<<L); i++) if(!app[i]) {
        string ans;
        for(int j = L-1; j >= 0; j--) ans += '0' + (i >> j & 1);
        cout << "YES\n" << string(k - L, '0') + ans << '\n';
        return;
    }
    cout << "NO\n";
