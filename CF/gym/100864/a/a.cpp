//   __________________
//  | ________________ |
//  ||          ____  ||
//  ||   /\    |      ||
//  ||  /__\   |      ||
//  || /    \  |____  ||
//  ||________________||
//  |__________________|
//  \###################\
//   \###################\
//    \        ____       \
//     \_______\___\_______\
// An AC a day keeps the doctor away.
 

#pragma GCC optimize("Ofast")
#pragma GCC target("sse3","sse2","sse")
#pragma GCC target("avx","sse4","sse4.1","sse4.2","ssse3")
#pragma GCC target("f16c")
#pragma GCC optimize("inline","fast-math","unroll-loops","no-stack-protector")
#pragma GCC optimize("Ofast")
#pragma loop_opt(on)
#include <bits/extc++.h>
#ifdef local
#define safe std::cerr<<__PRETTY_FUNCTION__<<" line "<<__LINE__<<" safe\n"
#define debug(...) qqbx(#__VA_ARGS__, __VA_ARGS__)
template <typename H, typename ...T> constexpr void qqbx(const char *s, const H &h, T &&...args) {
    for(; *s && *s != ','; ++s) if(*s != ' ') std::cerr << *s;
    std::cerr << " = " << h << (sizeof...(T) ? ", " : "\n");
    if constexpr(sizeof...(T)) qqbx(++s, args...);
}
#else
#include <bits/stdc++.h>
#define debug(...) ((void)0)
#define safe ((void)0)
#endif // local
#define all(v) begin(v),end(v)
#define get_pos(v,x) int(lower_bound(begin(v),end(v),x)-begin(v))
#define sort_uni(v) sort(begin(v),end(v)),v.erase(unique(begin(v),end(v)),end(v))
#define pb emplace_back
#define ff first
#define ss second
#define mem(v,x) memset(v,x,sizeof v)
 
using namespace std;
using namespace __gnu_pbds;
typedef int64_t ll;
typedef long double ld;
typedef pair<ll,ll> pll;
typedef pair<ld,ld> pld;
template <typename T> using max_heap = std::priority_queue<T,vector<T>,less<T> >;
template <typename T> using min_heap = std::priority_queue<T,vector<T>,greater<T> >;
template <typename T> using rbt = tree<T,null_type,less<T>,rb_tree_tag,tree_order_statistics_node_update>;
constexpr ld PI = acos(-1), eps = 1e-7;
constexpr ll N = 1000025, INF = 1e18, MOD = 1000000007, K = 14699, inf = 1e9;
constexpr inline ll cdiv(ll x, ll m) { return x/m + ((x<0 ^ m>0) && (x%m)); } // ceiling divide
constexpr inline ll modpow(ll e,ll p,ll m=MOD) { ll r=1; for(e%=m;p;p>>=1,e=e*e%m) if(p&1) r=r*e%m; return r; }
int arr[1000005], vis[1000005];
vector<int> ord,E[1000005];

void dfs(int x){
    vis[x] =1;
    for(auto i:E[x]){
        if(!vis[i])dfs(i);
    }
    ord.pb(x);
}


#define se second
#define fi first


signed main() {
#ifndef local
    freopen("agnatic.in", "r", stdin);
    freopen("agnatic.out", "w", stdout);
#endif
    ios_base::sync_with_stdio(0), cin.tie(0);
    int n, ok = 1;
    cin >> n;
    map<pair<char,char>, vector<int>> mp;
    for(int i = 0; i < n; i++) {
        string s, t;
        cin >> s;
        t = s;
        sort_uni(t);
        for(char c: t) {
            c-='A';
            for(char d: t) if(c < d) {
                vector<int> pos;
                d -= 'A';
                for(char e: s) {
                    e -= 'A';
                    if(e == c || e == d) pos.pb(e);
                }
                if(mp.count({c,d}) && mp[{c,d}] != pos) ok = 0;
                else mp[{c,d}] = pos;
            }
        }
    }
    if(!ok) return cout << "NO\n", 0;
    vector<int> in(10000025);
    string ans = "";
    map<int,int> id[26];
    int idx = 0;
    for(auto i:mp){
        int cnt[26] ={0};
        for(int j=0;j<i.se.size();j++){
            int x = i.se[j];
            if(!id[x][++cnt[x]]){
                id[x][cnt[x]] = ++idx;
                arr[idx] = x;
            }
            if(j){
                int k = i.se[j-1], cc = cnt[k] - (k==x);
                E[id[k][cc]].pb(id[x][cnt[x]]);
                in[id[x][cnt[x]]]++;
            }
        }
    }
    queue<int> q;
    for(int i = 1; i <= idx; i++) if(in[i] == 0) q.push(i);
    while(!q.empty()) {
        int i = q.front(); q.pop();
        ans += char(arr[i]+'A');
        for(int j: E[i]) if(--in[j] == 0) q.push(j);
    }
    for(int i = 1; i <= idx; i++) if(in[i]) return cout << "NO\n", 0;
    cout << "YES\n";
    cout<<ans<<'\n';
}
