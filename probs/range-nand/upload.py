...
def login():
    ...
    print('logging in...')
    global session
    rel = session.get('https://judge.ioicamp.org/users/sign_in')
    soup = BeautifulSoup(rel.text, "html.parser")
    inputs = soup.find('form').find_all('input')
    rel = session.post('https://judge.ioicamp.org/users/sign_in', data = {
...
sign_up_get_url = 'https://judge.ioicamp.org/problems/%s/testdata/new' % problem_id
sign_up_post_url = 'https://judge.ioicamp.org/problems/%s/testdata' % problem_id
